...
public class Parser {
    ...
    private Statement setConditionalStatement() throws SyntaxException, SemanticException {
        ...
        if (isTypeMatch(Token.Type.LPAREN)) {
            ...
            if (isTypeMatch(Token.Type.RPAREN)) {
                current = get(-1);
                if (isTypeMatch(Token.Type.LBRACE)) {
                    IntermediateCode.setCondition_IfStart(expr, pointer);
                    while (true) {
                    ...
                    IntermediateCode.setCondition_IfEnd(pointer + 1);
                    current = get(-1);
                    if (isTypeMatch(Token.Type.RBRACE)) {
                        current = get(-1);
                        // else-операторы
                        if (isTypeMatch(Token.Type.ELSE)) {
                            current = get(-1);
                            if (isTypeMatch(Token.Type.LBRACE)) {
                                IntermediateCode.setCondition_ElseStart(pointer);
                                while (true) {
                                ...
                                IntermediateCode.setCondition_ElseEnd(pointer + 1);
                                current = get(-1);
                                if (!isTypeMatch(Token.Type.RBRACE)) {
                            ...
                            IntermediateCode.setCondition_ElsePass(pointer + 1);
                        }
                    } else {
                        throw new SyntaxException(String.format("Ожидалась \"}\" после \"%s\": %s", current.getText(),
                                current.getFullPosition()));
    ...
    private Statement setPrecyclicStatement() throws SemanticException, SyntaxException {
        ...
        if (isTypeMatch(Token.Type.LPAREN)) {
            ...
            expr = whileExpr;
            IntermediateCode.setСycle(expr);
            while (true) {
                current = get(-1);
                ...
                //IntermediateCode.setPrecycle_End(pointer);
                if (!(Boolean) whileExpr.getResult())
                    break;
                    // IntermediateCode.setPrecycle_End(whileExpr, pointer);
                    // IntermediateCode.setStop(true);
                globalPos = whileExprPos;
                whileExpr = setExpression();
            }
            //IntermediateCode.setPrecycle_End(expr, pointer);
        } else {
