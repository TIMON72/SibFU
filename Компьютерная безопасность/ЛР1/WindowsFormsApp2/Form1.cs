...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        /// <summary>
        /// Ф-ция дополнительного шифрования текста
        /// </summary>
        /// <param name="text"></param>
        /// <returns></returns>
        private string AdditionEncryption(string text)
        {
            string result = "";
            for (int i = 0, j = 0, k = size - 1; i < text.Length; i++, j++, k--)
            {
                if (k == 0)
                    k = size - 1;
                int t = i % key.Length;
                ...
                int[] chPos = FindCharacterPosition(ch); // [0] - строка, [1] - столбец
                // Если строка со смещением на j выходит за пределы размерности матрицы, то
                if (chPos[0] + j >= size)
                {
                    chPos[0] = j - (size - chPos[0]);
                }
                // Иначе просто смещаем
                else
                {
                    chPos[0] = chPos[0] + j;
                }
                // Если строка со смещением на k выходит за пределы размерности матрицы, то
                if (chPos[0] + k >= size)
                {
                    chPos[0] = k - (size - chPos[0]);
                }
                // Иначе просто смещаем
                else
                {
        ...
        /// <summary>
        /// Ф-ция дополнительного расшифрования текста
        /// </summary>
        /// <param name="text"></param>
        /// <returns></returns>
        private string AdditionDecryption(string text)
        {
            string result = "";
            for (int i = 0, j = 0, k = size - 1; i < text.Length; i++, j++, k--)
            {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            text = richTextBox2.Text.ToLower();
            // Расшифруем текст (шаг 2, 3, 4)
            text = Decryption(text);

            text = AdditionDecryption(text);
            // Выводим на textBox4
