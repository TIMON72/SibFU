...
namespace MLP
{
    public static class Perceptron
    {
        ...
        static void ReadFileCSV(string resourceFileName, List<string[]> inputData, List<string> outputData, bool clear)
        {
            ...
            Data.Normalization(ref inputData, ref outputData); // Нормализуем входные и выходные параметры
        }
        /// <summary>
        ...
        {
            //ReadFileCSV("X_train", inputTeachingData, outputTeachingData, true);
            //ReadFileCSV("X_test", inputRealData, outputRealData, false);
            //ReadFileCSV("X_teaching", inputTeachingData, outputTeachingData, true);
            //ReadFileCSV("X_checking", inputRealData, outputRealData, false);
        ...
        /// <summary>
        /// Предсказание выходных значений и запись в файл
        /// </summary>
        /// <returns></returns>
        static string Predict()
        {
            ...
            // Даем нейросети получить ответ и денормализовать его
            do
            {
                double answer = Answer(ConvertInputStringToInputDouble(inputRealData[counter]));
                string denormalizeAnswer = Data.OutputDenormalization(answer.ToString());
                results.Add(denormalizeAnswer);
                counter++;

            } while (counter != inputRealData.Count - 1);
            // Запись в файл
            using (FileStream fs = new FileStream(@"X_result.csv", FileMode.Create))
            {
                byte[] input;// = Encoding.Default.GetBytes("Y;");
                //fs.Write(input, 0, input.Length);
                foreach (string r in results)
                {
                    input = Encoding.Default.GetBytes(r + "\n");
                    fs.Write(input, 0, input.Length);
                }
