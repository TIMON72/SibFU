...
type
  TForm1 = class(TForm)
    ...
    procedure ButtonCallProcedureClick(Sender: TObject);
...
procedure TForm1.ButtonCallProcedureClick(Sender: TObject);
var
  QueryString, DateBeg, DateEnd: AnsiString;
begin
  DateBeg := QuotedStr(FormatDateTime('YYYY-MM-DD', DateTimePicker1.Date));
  DateEnd := QuotedStr(FormatDateTime('YYYY-MM-DD', DateTimePicker2.Date));
  QueryString := 'CALL getgoods(' + DateBeg + ',' + DateEnd + ');';
  try
    with ADOQuery1 do
    begin
      SQL.Clear;
